...
class PromptMonitorBehaviorTests(unittest.TestCase):
    ...
    def test_run_single_flight_uses_activity_seen_during_submit_probe(self) -> None:
        cfg = tool.Config(
            prompt="x",
            max_prompts=2,
            poll_seconds=0.0,
        ...
        mon = tool.PromptMonitor(cfg)
        submit_activity = [True, False]

        def submit_with_activity(_window: object) -> bool:
            mon._last_submit_saw_activity = submit_activity.pop(0) if submit_activity else False
            return True
        ...
        with (
            ...
            patch.object(mon, "_chat_active_source", return_value=None),
            patch.object(mon, "_chat_output_fingerprint", return_value=("done",)),
            patch.object(mon, "_submit_prompt", side_effect=submit_with_activity) as submit_mock,
            patch("x_trigger_prompt_x.time.sleep", return_value=None),
        ):
            mon.run()
    ...
    def test_run_single_flight_dry_run_does_not_wait_for_output_snapshot(self) -> None:
        ...
        with (
            ...
            patch.object(mon, "_should_halt", return_value=False),
            patch.object(mon, "_chat_active_source", return_value=None),
            patch.object(mon, "_chat_output_fingerprint", return_value=None) as fingerprint_mock,
            patch.object(mon, "_submit_prompt", return_value=True) as submit_mock,
            patch("x_trigger_prompt_x.time.sleep", return_value=None),
        ):
            mon.run()
    ...
    def test_run_single_flight_non_dry_run_keeps_extended_no_activity_backoff_log(self) -> None:
        cfg = tool.Config(
            prompt="x",
            max_prompts=1,
            poll_seconds=0.0,
        ...
        messages = [call.args[0] for call in log_mock.call_args_list]
        self.assertTrue(any(msg.startswith("Submitted 1/1.") for msg in messages))
        self.assertTrue(any("No post-submit activity detected" in msg for msg in messages))
